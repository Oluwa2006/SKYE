...
export async function GET(req: NextRequest) {
  ...
  if (!renderId || !bucketName) {
    return NextResponse.json({ error: "render_id and bucket_name are required" }, { status: 400 });
  }
  ...
  try {
    ...
    if (progress.fatalErrorEncountered) {
      if (variantId) {
        await supabase.from("variant_outputs").update({ render_status: "failed" }).eq("id", variantId).catch(() => {});
      }
    ...
    if (progress.done) {
      ...
      if (variantId && outputUrl) {
        await supabase.from("variant_outputs").update({ render_status: "done", final_video_url: outputUrl }).eq("id", variantId).catch(() => {});
      }
